...
uniform float uTime,uScale,uSpeed,uPulse,uTurbulence,uDeformation,uPhase,uAppendages,uAspectX,uAspectY,uBody,uPointerX,uPointerY,uParticleSize;
...
void main(){
  ...
  // Decomposed reference field: harmonic frequencies -> radial magnitude -> nonlinear exponent -> temporal modulation.
  ...
  else if(uBody<2.5){float arm=floor(u*uAppendages);float a=(arm/max(uAppendages,1.0))*PI*2.0;float q=fract(u*uAppendages);pos=vec2(cos(a),sin(a))*(.25+q*1.25);pos+=vec2(sin(q*9.+t+phase),cos(q*7.-t*.8))*q*.14;}
  ...
  float wave=sin(pos.x*6.+phase+t)*.08+cos(pos.y*7.-t*.8+phase)*.06;
  vec2 flow=vec2(sin(t+phase+pos.y*5.),cos(t*.7+phase+pos.x*6.))*uTurbulence*.18;
  pos+=flow+vec2(wave,-wave*.35)*uDeformation;
  pos+=vec2(k*.002,e*.002)*parametricDeformation*uDeformation;
  pos*=uScale*(1.+sin(t*1.5+phase)*.08*uPulse);
...
void main(){vec2 p=gl_PointCoord-.5;float d=length(p)*2.;float core=pow(max(0.,1.-d),1.6);float halo=pow(max(0.,1.-d),.55)*.3*uGlow;vec3 c=mix(uColorA,uColorB,vHue);c=mix(c,uColorC,vLife*.35);c=hue(c,uHue);c*=uBrightness;float alpha=(core+halo)*(.35+.65*(1.-vLife*.35));gl_FragColor=vec4(c*uSaturation,alpha);}`;
