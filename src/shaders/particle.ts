...
uniform float uTime,uScale,uSpeed,uPulse,uTurbulence,uDeformation,uPhase,uAppendages,uAspectX,uAspectY,uBody,uMotion,uSymmetry,uMorphology,uPointerX,uPointerY,uParticleSize;
...
void main(){
  ...
  if(uMorphology>.5){
    ...
    float arm=floor(u*uAppendages);float a=(arm/max(uAppendages,1.))*PI*2.;float q=v;float curl=sin(q*8.-t*uMotion+phase)*q*.22;pos=vec2(cos(a+curl),sin(a+curl))*(.18+q*1.15);
  } else if(uBody<3.5){
    ...
    float y=v*2.-1.;float segment=floor(v*7.);float wobble=sin(segment*1.7+t*uMotion+phase)*.08;pos=vec2(wobble,y*1.42);
    if(u>.62){float side=step(.81,u)*2.-1.;float q=fract(u*5.);pos+=vec2(side*(.28+.42*q),-.12*q+sin(q*9.+t)*.08);}
  } else {
    float y=v*2.-1.;float spine=.42*sin(y*3.2)+.12*sin(y*9.);pos=vec2(spine,y*1.35);
  ...
  float anatomyStability=1.-uMorphology*.72;
  ...
  pos+=flow*anatomyStability+vec2(wave+symmetryRipple,-wave*.35)*uDeformation*anatomyStability;
  pos+=vec2(k*.0017,e*.0017)*parametricDeformation*uDeformation*anatomyStability;
  ...
  vec4 mv=modelViewMatrix*vec4(pos,depth,1.);
  gl_PointSize=clamp((1.+hash(seed*8.)*2.)*uParticleSize*(180./max(-mv.z,.1)),1.,18.);
  gl_Position=projectionMatrix*mv;
  vLife=v;vDepth=mv.z;vHue=fract(seed*.17+u*.2+aPart*.013);
}`;
